...
if (!class_exists("nxs_class_SNAP_TG")) { class nxs_class_SNAP_TG {
	...
	function doPostToNT($options, $message){ global $nxs_gCookiesArr; $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>'');
        ...
        if (!isset($options['webPrev'])) $options['webPrev'] = 1;
        ...
        if ($options['attchImg'] == '1') {
            $url = 'https://api.telegram.org/bot' . $options['botTkn'] . '/sendPhoto';
            $flds = array('chat_id' => $options['whToPost'], 'photo' => $imgURL, 'caption' => '');
            $hdrsArr = $this->nxs_getHeaders('https://api.telegram.org', true);
            $advSet = nxs_mkRemOptsArr($hdrsArr, '', $flds);
            $ret = nxs_remote_post($url, $advSet);
            if (is_nxs_error($ret)) {
                $badOut = print_r($ret, true) . " - ERROR";
                return $badOut;
            }
        }
        $msg = str_ireplace('<strong>', '<b>', str_ireplace('</strong>', '</b>', str_ireplace('<em>', '<i>', str_ireplace('</em>', '</i>', $msg))));
        $msg = nsTrnc(strip_tags($msg, '<b><i><a><code><pre>'), 3000);
        $url = 'https://api.telegram.org/bot' . $options['botTkn'] . '/sendMessage';
        $flds = array('chat_id' => $options['whToPost'], 'text' => $msg, 'parse_mode' => 'HTML', 'disable_web_page_preview' => $options['webPrev'] == '0');
        $hdrsArr = $this->nxs_getHeaders('https://api.telegram.org', true);
        $advSet = nxs_mkRemOptsArr($hdrsArr, '', $flds);
        $ret = nxs_remote_post($url, $advSet);
        if (is_nxs_error($ret)) {
        ...
        $contents = $ret['body'];
        $resp = json_decode($contents, true);    //  prr($resp);
        if (is_array($resp) && !empty($resp['ok']) && $resp['ok'] == 1) {
            if (empty($resp['result']['chat']['username'])) $purl = 'https://web.telegram.org/#/im?p=' . str_replace('-', 'g', $resp['result']['chat']['id']); else $purl = 'http://telegram.me/' . $resp['result']['chat']['username'];
