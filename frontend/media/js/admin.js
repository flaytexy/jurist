...
function getModalSelectAttachment () {
    $('.modal-backdrop').remove();
    $('#modal-select-attachment').remove();

    $.get(
        '/admin/attachment/default/index',
        {
            modal: true,
            onlyImages: true
        },
        function (data) {
            ...
            $('body').append(data);
            $('#attachment-select').data('$input', $(this).closest('.card-block').find('input[type="hidden"]'));
            $('#modal-select-attachment').modal('show');
        }
    );
}
...
$(function() {
    ...
    $("#phone-add").on('click', function(e) {
        ...
        var phone_input = $(this).closest('.card').find('input[type="text"]:last'),
            phone_input_clone = phone_input.clone();

        phone_input_clone.val('');

        phone_input.after(phone_input_clone);
    });

    $(".password-toggle .input-group-addon").on('click', function() {
        var $password_container = $(this).parents('.password-toggle');

        if ($password_container.find('.fa').hasClass('fa-eye-slash')) {
            $password_container.find('.fa').removeClass('fa-eye-slash').addClass('fa-eye');
            $password_container.find('input').attr('type', 'text');
        } else {
            $password_container.find('.fa').removeClass('fa-eye').addClass('fa-eye-slash');
            $password_container.find('input').attr('type', 'password');
        }
    });

    $(document).on('click', '#image-manager-set', function (e) {
        e.preventDefault();
        getModalSelectAttachment();
    });

    $(document).on('click', '#attachment-select', function (e) {
        e.preventDefault();

        if ($(this).data('gallery')) {
            $('.b-thumb.active').each(function () {
                $('.car-images').append('<div class="car-image col-md-3">' + $('.b-thumb__preview__centered', this).html() + '<div class="car-image-controls"><i class="fa fa-arrows"></i><i class="fa fa-remove"></i></div><input type="hidden" name="Images[]" value="' + $(this).data('id') + '"></div>');
            });
        } else {
            $('#image-manager-thumbnail').val($('.b-thumb.active').data('id'));
            $('#image-manager-preview').html($('.b-thumb.active .b-thumb__preview__centered').html());

            $('#image-manager-reset').removeClass('hidden');
            $('#image-manager-set').addClass('hidden');
        }

        $('#modal-select-attachment').modal('hide');
    });

    $(document).on('click', '#image-manager-set2', function (e) {
        e.preventDefault();

        var $image_manager = $('#image-manager2'),
            $card = $image_manager.parents('.card-block');

        $image_manager.redactor('focus.setStart');

        $card.find('.re-image').click();
    });

    $(document).on('click', '#image-manager-reset', function (e) {
        e.preventDefault();

        $('#image-manager-preview').html('');

        $('#image-manager-thumbnail').val('');

        $('#image-manager-reset').addClass('hidden');
        $('#image-manager-set').removeClass('hidden');
    });

    $(document).on('click', '#image-manager-reset2', function (e) {
        e.preventDefault();

        $('#image-manager-preview2').html('');

        $('#image-manager-thumbnail2').val('');

        $('#image-manager-reset2').addClass('hidden');
        $('#image-manager-set2').removeClass('hidden');
    });

    $(document).on('click', '.car-image-controls .fa-remove', function (e) {
        e.preventDefault();

        $(this).closest('.car-image').remove();
    });

    $(document).on('click', '#image-manager-gallery-set', function (e) {
        e.preventDefault();
        ...
        $.get(
            ...
            function (data) {
                $('#modal-select-attachment').remove();
                $('body').append(data);
                $('#attachment-select').data('gallery', true);
                $('#modal-select-attachment').removeClass('fade');
                $('#modal-select-attachment').modal('show');
            }
        );
    });
...
function imageManagerUpload($image, json) {
    var filename = typeof json === 'string' ? $(json).attr('src') : json.filelink;

    var $image_manager = $('#image-manager'),
        $card = $image_manager.parents('.card-block');

    $('#image-manager-preview').html('<img src="' + filename + '">');

    $('#image-manager-thumbnail').val(filename.replace('/uploads/', ''));

    $('#image-manager-set').addClass('hidden');
    $('#image-manager-reset').removeClass('hidden');
}

function imageManagerUpload2($image, json) {
    var filename = typeof json === 'string' ? $(json).attr('src') : json.filelink;

    var $image_manager = $('#image-manager2'),
        $card = $image_manager.parents('.card-block');

    $('#image-manager-preview2').html('<img src="' + filename + '">');

    $('#image-manager-thumbnail2').val(filename.replace('/uploads/', ''));

    $('#image-manager-set2').addClass('hidden');
    $('#image-manager-reset2').removeClass('hidden');
}
