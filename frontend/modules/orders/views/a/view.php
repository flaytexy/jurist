...
$this->title = Yii::t('easyii/orders', 'View orders');
$this->registerCss('.orders-view dt{margin-bottom: 10px;}');

if($model->status == Orders::STATUS_ANSWERED) {
    $this->registerJs('$(".send-answer").click(function(){return confirm("'.Yii::t('easyii/orders', 'Are you sure you want to resend the answer?').'");})');
}
?>
<?= $this->render('_menu', ['noanswer' => $model->status == Orders::STATUS_ANSWERED]) ?>

<dl class="dl-horizontal orders-view">
