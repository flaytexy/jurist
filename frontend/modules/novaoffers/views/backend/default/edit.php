 ...
 * @var \frontend\modules\novaoffers\models\NovaoffersTranslation|array $translation_models
 */
        ...
        <div class="card card-block">
            ...
            foreach ($translation_models as $language => $translation_model) {
                echo $form->field($translation_model, "[$language]public_status", ['options' => ['class' => 'form-group language_' . $language . ($model->language !== $language ? ' hidden' : '')]])
                    ...
                    ->dropDownList(
                        [
                            Novaoffers::STATUS_OFF => 'Черновик',
                            Novaoffers::STATUS_ON => 'Опубликовано',
                        ],
                        [
            ...
            <?php if (isset($model->slug)) :?>
                <a class="btn btn-primary btn-block" target="_blank" href="<?=Url::to(['/offers/'.$model->slug]) ?>">Посмотреть запись </a>
            <?php   endif;?>
        </div>
...
<!--        </div>-->
        ...
        <?php if (false) : ?>
            ...
            <div>
                <?php
                // http://loc.ak.yabloko.studio/admin/novaoffers/default/cropImage?id=dfjksjkfsfd
