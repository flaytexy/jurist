 ...
 *
 * @method static TickersObject get(mixed $id_slug) Get tickers object by id or slug
 * @method static array items(array $options = []) Get list of tickers as TickersObject objects
 * @method static mixed clear() Clear all items
 * @method static mixed last(int $limit = 1) Get last tickers
 ...
 */
class Tickers extends \frontend\components\API
{
    private $_adp;
    private $_last;
    private $_items;
    private $_item = [];
    private $_default_model = 'TickersModel';
    ...
    public function api_items($options = [])
    {
        ...
        if (!($this->_items && $this->_adp)) {
            ...
            if (Yii::$app->getModule('admin')->activeModules['tickers']->settings['enableTags']) {
                $with[] = 'tags';
            }

            $query = TickersModel::find()
                ->with($with)
                ->status(TickersModel::STATUS_ON);

            if (!empty($options['list'])) {

                $query->select(" " . TickersModel::tableName() . ".*, cdt.* , ca.`country_id` as ca_id, cra.*, cr.name as region_name  ");

                $query->join(
                    'LEFT JOIN',
                    'country_assign as ca',
                    " `ca`.`item_id` = `" . TickersModel::primaryKey()[0] . "` AND `ca`.`class` LIKE '" . addslashes(addslashes(TickersModel::className())) . "'  "
                );
...
//                )->andWhere(" `cr`.`is_unep` = '1' ");
            ...
            if (!empty($options['tags'])) {
                $query
                    ...
                    ->andWhere([Tag::tableName() . '.name' => (new TickersModel)->filterTagValues($options['tags'])])
                    ->addGroupBy('ticker_id');
            }
            ...
            /**
             * @var TickersModel $model
             */
            foreach ($this->_adp->models as $model) {
                $item = new TickersObject($model);
                //$item->countries = isset($countries[$model->ticker_id]) ? $countries[$model->ticker_id] : $model->countries;
                $this->_items[] = $item;
            }
    ...
    public function api_get($id_slug)
    {
        if (!isset($this->_item[$id_slug])) {
            $this->_item[$id_slug] = $this->findTickers($id_slug);
        }
    ...
    public function api_last($limit = 1)
    {
        ...
        $result = [];
        foreach (TickersModel::find()->with($with)->status(TickersModel::STATUS_ON)->sortDate()->limit($limit)->all() as $item) {
            $result[] = new TickersObject($item);
        }
    ...
    private function findTickers($id_slug)
    {
        $tickers = TickersModel::find()
            ->where(['or', 'ticker_id=:id_slug', 'slug=:id_slug'], [':id_slug' => $id_slug])
            ->status(TickersModel::STATUS_ON)
            ->one();
        if ($tickers) {
            $tickers->updateCounters(['views' => 1]);
            return new TickersObject($tickers);
        } else {
            return null;
        }
    }
