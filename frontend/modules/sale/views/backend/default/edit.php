 ...
 * @var \frontend\modules\sale\models\SaleTranslation|array $translation_models
 */
        ...
        <div class="card card-block">
            ...
            foreach ($translation_models as $language => $translation_model) {
                echo $form->field($translation_model, "[$language]public_status", ['options' => ['class' => 'form-group language_' . $language . ($model->language !== $language ? ' hidden' : '')]])
                    ...
                    ->dropDownList(
                        [
                            Sale::STATUS_OFF => 'Черновик',
                            Sale::STATUS_ON => 'Опубликовано',
                        ],
                        [
            ...
            <?php if (isset($model->slug)) :?>
                <a class="btn btn-primary btn-block" target="_blank" href="<?=Url::to(['/sale/'.$model->slug]) ?>">Посмотреть запись </a>
            <?php   endif;?>
        </div>
...
<!--        </div>-->
        ...
        <?php if (false) : ?>
            ...
            <div>
                <?php
                // http://loc.ak.yabloko.studio/admin/sale/default/cropImage?id=dfjksjkfsfd
