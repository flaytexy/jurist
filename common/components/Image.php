...
class Image extends \yii\imagine\Image
{
    ...
    /**
     * Strict resize an image by limit length(width, heigth) .
     *
     * @param $filename
     * @param integer $limitSize
     * @param array $paramsSave
     * @param string $outputFile
     * @param string $filter
     * @return static
     */
    public static function resizeByMaxLength($filename, $limitSize = 1280, $paramsSave = array(), $outputFile = null , $filter = \Imagine\Image\ImageInterface::FILTER_UNDEFINED)
    {
        list($width, $height, $type, $attr) = getimagesize($filename);

        if($width > $limitSize || $height > $limitSize){
            if ($width > $height) {
                $image = self::resizeByWidth($filename, $limitSize);
            } else {
                $image = self::resizeByHeight($filename, $limitSize);
            }
        }else{
            $image = static::getImagine()
                ->open(\Yii::getAlias($filename))
                ->resize(new Imagine\Image\Box($width, $height), $filter);
        }

        return $image->save($outputFile, $paramsSave);
    }
