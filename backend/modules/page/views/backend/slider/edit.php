...
<div class="item-editor-page">
    ...
    <div class="title-block">
        <h3 class="title"> <?= $model->isNewRecord ? 'Добавить' : 'Редактировать' ?> слайд <span class="sparkline bar" data-type="bar"></span></h3>
    </div>
    ...
    <div class="col-sm-3 push-sm-9">
            ...
            <?php
                foreach ($translation_models as $language => $translation_model) {
                    echo $form->field($translation_model, "[$language]status", ['options' => ['class' => 'form-group language_' . $language . ($model->language !== $language ? ' hidden' : '')]])
                        ...
                        ->dropDownList(
                            [
                                Slider::STATUS_DRAFT => 'Черновик',
                                Slider::STATUS_PUBLISHED => 'Опубликовано',
                            ],
                            [
        ...
        <div class="card card-block">
            ...
            <div class="file-input">
                ...
                <a href="#" id="image-manager-set"<?= $model->thumbnail ? ' class="hidden"' : ''; ?>>Задать слайд</a>
                <a href="#" id="image-manager-reset"<?= !$model->thumbnail ? ' class="hidden"' : ''; ?>>Удалить слайд</a>
            </div>
    ...
    <div class="col-sm-9 pull-sm-3">
        <div class="card card-block">
            ...
            foreach ($translation_models as $language => $translation_model) {
                ...
                echo $form->field($translation_model, "[$language]button_text", ['options' => ['class' => 'form-group language_' . $language . ($model->language !== $language ? ' hidden' : '')]])
                    ->label(null, ['class' => 'form-control-label'])
