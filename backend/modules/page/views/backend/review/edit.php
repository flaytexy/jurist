...
<div class="item-editor-page">
    ...
    <div class="title-block">
        <h3 class="title"> <?= $model->isNewRecord ? 'Добавить' : 'Редактировать' ?> отзыв <span class="sparkline bar" data-type="bar"></span></h3>
    </div>
            ...
            <?php
                foreach ($translation_models as $language => $translation_model) {
                    echo $form->field($translation_model, "[$language]status", ['options' => ['class' => 'form-group language_' . $language . ($model->language !== $language ? ' hidden' : '')]])
                        ...
                        ->dropDownList(
                            [
                                ReviewTranslation::STATUS_DRAFT => 'Черновик',
                                ReviewTranslation::STATUS_PUBLISHED => 'Опубликовано',
                            ],
                            [
    ...
    <div class="col-sm-9 pull-sm-3">
        <div class="card card-block">
            ...
            foreach ($translation_models as $language => $translation_model) {
                ...
                echo $form->field($translation_model, "[$language]youtube_link", ['options' => ['class' => 'form-group language_' . $language . ($model->language !== $language ? ' hidden' : '')]])
                    ->label(null, ['class' => 'form-control-label'])
