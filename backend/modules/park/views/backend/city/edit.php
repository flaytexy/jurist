...
<div class="item-editor-page">
    ...
    <div class="title-block">
        <h3 class="title"> <?= $model->isNewRecord ? 'Добавить' : 'Редактировать' ?> город <span class="sparkline bar" data-type="bar"></span></h3>
    </div>
    ...
    <div class="col-sm-3 push-sm-9">
        ...
        <div class="card card-block">
            ...
            <?=
            $form->field($model, 'show_in_header', ['options' => ['template' => '{input}']])
                ->checkbox([
                    'class' => 'radio squared',
                    'label' => '<span>' . $model->getAttributeLabel('show_in_header') . '</span>'
                ]);
            ?>

            <?=
            $form->field($model, 'show_in_footer')
                ->checkbox([
                    'class' => 'radio squared',
                    'label' => '<span>' . $model->getAttributeLabel('show_in_footer') . '</span>'
                ]);
            ?>
        ...
        <div class="card card-block">
            <div class="form-group field-city-phones">
                <label class="form-control-label"><?= $model->getAttributeLabel('phones') ?></label>
                <?php foreach ($model->_phones as $phone) { ?>
                    <?= Html::input('text', '_phones[]', $phone, ['class' => 'form-control boxed']) ?>
                <?php } ?>
            </div>
            <a href="#" id="phone-add"><small>добавить</small></a>
        </div>
