...
class ImageUploaderBehavior extends Behavior
{
    ...
    protected $_savePathAlias = '@frontend/web/uploads';
    ...
    private static $_prefixOriginal = ''; //_original
    ...
    protected function uploadImage(UploadedFile $image)
    {
        ...
        if ($image->saveAs($fullImagePath)) {
            ...
            if ($rez === true) {
                // Сохраняем оригинал
                $originalImage = $imageFolder . $DS . $rnddir . $DS . $namePart . self::$_prefixOriginal . '.' . $image->extension;
    ...
    protected function getOriginalImagePath()
    {
        ...
        return static::addPostfixToFile($savePath . DIRECTORY_SEPARATOR . $image, self::$_prefixOriginal);
    }
