...
class LanguageRequest extends Request
{
    ...
    public function getLanguageUrl()
    {
        if ($this->_language_url === null) {
            ...
            $language_url = !empty($url_list[1]) ? $url_list[1] : null;

            Language::setCurrent($language_url);

            if ($language_url !== null && $language_url === Language::getCurrent()['id'] &&
                strpos($this->_language_url, Language::getCurrent()['id']) === 1
            ) {
                $this->_language_url = substr($this->_language_url, strlen(Language::getCurrent()['id']) + 1);
            }
        }
